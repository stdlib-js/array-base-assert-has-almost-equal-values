...
var isAlmostEqualF16 = require( '@stdlib/number-float16-base-assert-is-almost-equal' );
...
function hasAlmostEqualValues( x, y, maxULP ) {
	...
	if ( xo.accessorProtocol || yo.accessorProtocol ) {
		...
		if ( FLG === 0 ) {
			// Determine whether we should use double-precision, single-precision or half-precision comparison...
			if ( xr.BYTES_PER_ELEMENT === 8 || yr.BYTES_PER_ELEMENT === 8 ) {
				return internal( xr, yr, maxULP, isAlmostEqualF64 );
			}
			if ( xr.BYTES_PER_ELEMENT === 4 || yr.BYTES_PER_ELEMENT === 4 ) {
				return internal( xr, yr, maxULP, isAlmostEqualF32 );
			}
			return internal( xr, yr, maxULP, isAlmostEqualF16 );
		}
		return accessors( xo, yo, maxULP, isAlmostEqual ); // general accessors should always use general comparison
	}
	// Determine whether we should use double-precision, single-precision, or half-precision comparison...
	if ( x.BYTES_PER_ELEMENT === 2 && y.BYTES_PER_ELEMENT === 2 ) {
		return internal( x, y, maxULP, isAlmostEqualF16 );
	}
